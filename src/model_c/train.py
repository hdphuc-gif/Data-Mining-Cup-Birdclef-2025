...
def train(model, train_loader, criterion, optimizer, device, num_classes=206):
    ...
    for inputs, targets in train_loader:
        # Convert class index thành one-hot
...
def main(cfg):
    ...
    for epoch in range(cfg['training']['epochs']):
        ...
        val_loss = validate(model, val_loader, criterion, device, num_classes=206)
        ...
        if save_best:
            if val_loss < best_val_loss:
                ...
                torch.save(model.state_dict(), os.path.join(cfg['save_path'], "convnext_bird.pth"))
        else:
            torch.save(model.state_dict(), os.path.join(cfg['save_path'], f"model: convnext_bird_epoch{epoch+1}.pth"))

    if not save_best:
        torch.save(model.state_dict(), os.path.join(cfg['save_path'], "convnext_bird_final.pth"))
